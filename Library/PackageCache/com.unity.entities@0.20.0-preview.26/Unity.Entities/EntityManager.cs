...
#pragma warning restore 0618
        ...
        private void CreateJobReflectionData()
        {
            ...
            IJobBurstSchedulableExtensions.JobStruct<MoveFilteredChunksBetweenArchetypeJob>.Initialize();
            ...
            IJobBurstSchedulableExtensions.JobStruct<BurstableCollectionsHacks.WorkaroundUnsafeDisposeJob>.Initialize();
...
// TODO: remove UNITY_EDITOR conditional once https://unity3d.atlassian.net/browse/DOTS-3862 is fixed
            ...
            IJobBurstSchedulableExtensions.JobStruct<EntityPatcher.ApplyNamesJob>.Initialize();
#endif
        }
    ...
    // ----------------------------------------------------------------------------------------------------------
        ...
        public void AddComponent(NativeArray<Entity> entities, ComponentType componentType)
        {
            ...
            EntitiesJournaling.RecordAddComponent(in access->m_WorldUnmanaged, (Entity*)entities.GetUnsafeReadOnlyPtr(), entities.Length, &componentType.TypeIndex, 1);
#endif
...
#if DEVELOPMENT_BUILD || UNITY_EDITOR
            EntitiesJournaling.RecordAddComponent(in access->m_WorldUnmanaged, &entity, 1, types.Types, types.Length);
#endif
...
#if DEVELOPMENT_BUILD || UNITY_EDITOR
            EntitiesJournaling.RecordRemoveComponent(in access->m_WorldUnmanaged, &entity, 1, componentTypes.Types, componentTypes.Length);
#endif
        ...
        public void AddChunkComponentData<T>(EntityQuery entityQuery, T componentData) where T : unmanaged, IComponentData
        {
            ...
            if (chunks.Length > 0)
            {
                ...
                if (validAdd)
                {
                    ...
                    EntitiesJournaling.RecordAddComponent(in access->m_WorldUnmanaged, chunkPtr, chunks.Length, &componentTypeIndexForAdd, 1);
                    EntitiesJournaling.RecordSetComponentData(in access->m_WorldUnmanaged, chunkPtr, chunks.Length, &componentTypeIndexForAdd, 1, UnsafeUtility.AddressOf(ref componentData), UnsafeUtility.SizeOf<T>());
#endif
