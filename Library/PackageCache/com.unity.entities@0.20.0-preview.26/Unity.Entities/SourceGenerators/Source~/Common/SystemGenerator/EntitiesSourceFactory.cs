...
namespace Unity.Entities.SourceGen.SystemGeneratorCommon
{
    public static class EntitiesSourceFactory
    {
        ...
        static string EntityQueryFieldAssignment(EntityQueryField field, bool isInISystem)
        {
                ...
                $@"{field.FieldName} = {"systemState.".EmitIfTrue(isInISystem)}GetEntityQuery(
                    ...
                    {{
                        ...
                        Any = new Unity.Entities.ComponentType[] {{
                            {field.QueryDescription.Any.Select(GetReadOnlyQueryType).Distinct().SeparateByCommaAndNewLine()}
                        }},
                        None = new Unity.Entities.ComponentType[] {{
                            {field.QueryDescription.None.Select(GetReadOnlyQueryType).Distinct().SeparateByCommaAndNewLine()}
                        }},
            ...
            {
                entityQuerySetup +=
                    $@"{field.FieldName}.SetChangedVersionFilter(new ComponentType[{field.QueryDescription.ChangeFilterTypes.Count()}]
				    {{
                        {field.QueryDescription.ChangeFilterTypes.Select(withChangeFilterType =>
                            withChangeFilterType.isReadOnly
                                ? $"Unity.Entities.ComponentType.ReadOnly<{withChangeFilterType.typeInfo.Name}>()"
                                : $"Unity.Entities.ComponentType.ReadWrite<{withChangeFilterType.typeInfo.Name}>()").SeparateByComma()}
                    }});";
            }
        ...
        static string GetReadOnlyQueryType((INamedTypeSymbol typeInfo, bool isReadOnly) componentType)
        {
            return
