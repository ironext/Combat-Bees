...
namespace Unity.Entities.SourceGen.LambdaJobs
{
    static class LambdaJobsVerification
    {
        public static void Verify(this LambdaJobDescription description)
        {
            ...
            if ((description.Burst.IsEnabled || description.Schedule.Mode != ScheduleMode.Run) &&
                description.VariablesCaptured.Any(var => var.Type.ContainingType != null && var.Type.ContainingType.IsReferenceType))
            {
                foreach (var variable in description.VariablesCaptured.Where(var => var.Type.ContainingType != null && var.Type.ContainingType.IsReferenceType))
                    LambdaJobErrors.DC0001(Context, Location, variable.OriginalVariableName, variable.Type.ContainingType.Name);
                description.Success = false;
            }
            */
        ...
        static bool VerifyLambdaExpression(LambdaJobDescription description)
        {
            ...
            foreach (var invocationExpression in description.OriginalLambdaExpression.DescendantNodes().OfType<InvocationExpressionSyntax>())
            {
                ...
                var result = invocationExpression.CheckIsForEach(description.SemanticModel);
                if (result.Success)
                {
                    LambdaJobsErrors.DC0029(description.SystemGeneratorContext, invocationExpression.GetLocation(), result.Type);
                    success = false;
                }
            }
            ...
            if (description.Burst.IsEnabled || description.Schedule.Mode == ScheduleMode.Schedule || description.Schedule.Mode == ScheduleMode.ScheduleParallel)
            {
                ...
                {
                    LambdaJobsErrors.DC0023(description.SystemGeneratorContext, description.Location, param.Name, description.IsInSystemBase);
                    description.Success = false;
                }
                foreach (var param in description.ExecuteMethodParamDescriptions.OfType<ISharedComponentParamDescription>())
                {
                    LambdaJobsErrors.DC0019(description.SystemGeneratorContext, description.Location, param.TypeSymbol.Name, description.IsInSystemBase);
                    description.Success = false;
                }
            }
