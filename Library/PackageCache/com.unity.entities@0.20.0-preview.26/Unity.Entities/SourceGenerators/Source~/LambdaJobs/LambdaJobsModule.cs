    ...
    public class LambdaJobsModule : ISystemModule
    {
        ...
        public bool GenerateSystemType(SystemGeneratorContext systemGeneratorContext)
        {
            ...
            foreach (var lambdaJobDescription in lambdaJobDescriptions)
            {
                if (lambdaJobDescription.LambdaJobKind == LambdaJobKind.Entities)
                {
                    systemGeneratorContext.CreateUniqueQueryField(
                        new EntityQueryDescription(
                            lambdaJobDescription.WithSharedComponentFilterTypes.Select(symbol => (symbol, true)).Concat(
                                lambdaJobDescription.LambdaParameters
                                    ...
                                    .Concat(lambdaJobDescription.WithAllTypes.Select(typeSymbol => (typeSymbol, true)))).ToList(),
                            lambdaJobDescription.WithAnyTypes.Select(typeSymbol => (typeSymbol, true)).ToList(),
                            lambdaJobDescription.WithNoneTypes.Select(typeSymbol => (typeSymbol, true)).ToList(),
                            lambdaJobDescription.WithChangeFilterTypes.Select(typeSymbol => (typeSymbol, false)).ToList(),
                    ...
                    {
                        lambdaParameter.ComponentTypeHandleFieldName =
                            systemGeneratorContext.GetOrCreateComponentTypeField(lambdaParameter.TypeSymbol, lambdaParameter.QueryTypeIsReadOnly());
                    }
                }

                systemGeneratorContext.AddNewMember(EntitiesSourceFactory.LambdaJobs.JobStructFor(lambdaJobDescription));

                if (lambdaJobDescription.WithStructuralChangesAndLambdaBodyInSystem)
                    systemGeneratorContext.AddNewMember(EntitiesSourceFactory.LambdaJobs.LambdaBodyMethodFor(lambdaJobDescription));
                ...
                systemGeneratorContext.AddNewMember(EntitiesSourceFactory.LambdaJobs.CreateExecuteMethod(lambdaJobDescription));
            }
            ...
            {
                var lambdaJobDescriptionsInMethod = lambdaJobDescriptions.Where(desc => desc.ContainingMethod == methodDeclarationSyntax);
                var singletonDescriptionsInMethod = singletonAccessDescriptions.Where(desc => desc.ContainingMethod == methodDeclarationSyntax);

                if (!lambdaJobDescriptionsInMethod.Any() && !singletonDescriptionsInMethod.Any())
                    continue;

                foreach (var lambdaJobDescriptionInMethod in lambdaJobDescriptionsInMethod)
                {
                ...
                foreach (var singletonDescriptionInMethod in singletonDescriptionsInMethod)
