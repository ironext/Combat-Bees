...
{
    [Generator]
    public class JobEntityGenerator : ISourceGenerator
    {
        ...
        {
            if (!context.ParseOptions.PreprocessorSymbolNames.Contains("DOTS_EXPERIMENTAL"))
                return;
            ...
            foreach (var kvp in systemReceiver.JobCandidatesBySyntaxTree)
            {
                ...
                var generatedStructs = new List<MemberDeclarationSyntax>();

                foreach (var candidate in candidates)
                {
                    var jobEntityDescription = new JobEntityDescription(candidate, semanticModel);
                    if (jobEntityDescription.Valid)
                    {
                        var generatedStruct = jobEntityDescription.Generate();
                        generatedStructs.Add(generatedStruct);
                    }
                }
