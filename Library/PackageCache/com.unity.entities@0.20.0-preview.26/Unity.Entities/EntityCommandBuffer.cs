...
namespace Unity.Entities
{
    ...
    internal unsafe struct EntityCommandBufferData
    {
        ...
        internal bool AppendMultipleEntitiesComponentCommandWithObject(EntityCommandBufferChain* chain, int sortKey, ECBCommand op,
                EntityQuery entityQuery, object boxedComponent, ComponentType ctype)
        {
            var entities = entityQuery.ToEntityArray(m_Allocator);
            if (entities.Length == 0)
            {
        ...
        internal bool AppendMultipleEntitiesComponentCommandWithSharedValue<T>(EntityCommandBufferChain* chain, int sortKey, ECBCommand op,
                EntityQuery entityQuery, int hashCode, object boxedComponent) where T : struct, ISharedComponentData
        {
            var ctype = ComponentType.ReadWrite<T>();
    ...
    public unsafe partial struct EntityCommandBuffer : IDisposable
    {
        ...
        void PlaybackInternal(EntityDataAccess* mgr)
        {
            ...
            walker.processor.Init(mgr, m_Data);
#if ENABLE_UNITY_COLLECTIONS_CHECKS
...
#pragma warning restore 728
        ...
        internal struct PlaybackProcessor : IEcbProcessor
        {
            ...
            public void Init(EntityDataAccess* entityDataAccess, EntityCommandBufferData* data)
            {
            ...
            public void DestroyEntity(BasicCommand* header)
            {
                ...
                mgr->DestroyEntityInternalDuringStructuralChange(&entity, 1);
            }
            ...
            public void RemoveComponent(BasicCommand* header)
            {
                ...
                var entity = SelectEntity(cmd->Header.Entity, playbackState);
                mgr->RemoveComponentDuringStructuralChange(entity, ComponentType.FromTypeIndex(cmd->ComponentTypeIndex));
            }
            ...
            public void RemoveMultipleComponents(BasicCommand* header)
            {
                ...
                EntitiesJournaling.RecordRemoveComponent(in mgr->m_WorldUnmanaged, &entity, 1, componentTypes.Types, componentTypes.Length);
#endif
            ...
            public void CreateEntity(BasicCommand* header)
            {
                ...
                mgr->CreateEntityDuringStructuralChange(at, playbackState.CreateEntityBatch + index, cmd->BatchCount);
            }
            ...
            public void AddComponent(BasicCommand* header)
            {
                ...
                var entity = SelectEntity(cmd->Header.Entity, playbackState);
                mgr->AddComponentDuringStructuralChange(entity, componentType);
...
#if DEVELOPMENT_BUILD || UNITY_EDITOR
                EntitiesJournaling.RecordAddComponent(in mgr->m_WorldUnmanaged, &entity, 1, componentTypes.Types, componentTypes.Length);
#endif
            ...
            public void AddBuffer(BasicCommand* header)
            {
                ...
                var entity = SelectEntity(cmd->Header.Entity, playbackState);
                mgr->AddComponentDuringStructuralChange(entity, ComponentType.FromTypeIndex(cmd->ComponentTypeIndex));
            ...
            public void AddComponentForMultipleEntities(BasicCommand* header)
            {
                ...
                if (cmd->ComponentSize > 0)
                {
                    for (int i = 0; i < entities.Length; i++)
                    {
                        mgr->SetComponentDataRaw(entities[i], cmd->ComponentTypeIndex, cmd + 1, cmd->ComponentSize);
                    }
                }
            }
            ...
            public void AddManagedComponentData(BasicCommand* header)
            {
                ...
                mgr->SetComponentObject(entity, ComponentType.FromTypeIndex(cmd->ComponentTypeIndex), box);
            }
            ...
            public void AddComponentObjectForMultipleEntities(BasicCommand* header)
            {
                ...
                for (int len = entities.Length, i = 0; i < len; i++)
                {
                    mgr->SetComponentObject(entities[i], componentType, box);
                }
            }
        ...
        unsafe public struct ParallelWriter
        {
            ...
            /// <param name="sortKey">A unique index for each set of commands added to the command buffer
