...
namespace Unity.Entities
{
    internal unsafe partial struct EntityComponentStore
    {
        ...
        internal static string AppendDestroyedEntityRecordError(Entity e)
        {
            ...
            var records = EntitiesJournaling.Records.WithEntity(e.Index, e.Version);
            for (var i = records.Length - 1; i >= 0; i--)
            {
                if (records[i].RecordType == EntitiesJournaling.RecordType.DestroyEntity)
                    return
                        $" Entity ({e.Index}:{e.Version}) was previously destroyed by {records[i].ExecutingSystem}.";
            }
#endif
            return default;
        }
        ...
        internal static string AppendRemovedComponentRecordError(Entity e, ComponentType type)
        {
            ...
            for (var i = records.Length - 1; i >= 0; i--)
            {
                if (records[i].RecordType == EntitiesJournaling.RecordType.RemoveComponent)
                {
                    for (var j = 0; j < records[i].ComponentTypes.Length; j++)
                    {
                        if (records[i].ComponentTypes[j] == type)
                            return
                                $" Component {records[i].ComponentTypes[j]} was removed from entity ({e.Index}:{e.Version}) previously by {records[i].ExecutingSystem}.";
                    }
                }
            }
#endif
