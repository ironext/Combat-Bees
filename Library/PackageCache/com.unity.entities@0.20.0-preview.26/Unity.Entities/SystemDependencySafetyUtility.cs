...
namespace Unity.Entities
{
    ...
    unsafe static class SystemDependencySafetyUtility
    {
        public struct SafetyErrorDetails
        {
            ...
            internal static string GetSystemTypeNameFromSystemId(int systemId, out bool isJobComponentSystem)
            {
                SystemState* ptr = World.FindSystemStateForId(systemId);

                if (ptr == null)
                {
                    isJobComponentSystem = false;
                    return "unknown";
                }
                ...
                if (managed != null)
                {
                    isJobComponentSystem = typeof(JobComponentSystem).IsAssignableFrom(managed.GetType());
            ...
            {
                string systemName = GetSystemTypeNameFromSystemId(m_ProblematicSystemId, out var isJobComponentSystem);
                int type = m_ProblematicTypeIndex;
                AtomicSafetyHandle h = m_ProblematicHandle;
                string errorTail = isJobComponentSystem ?
                    "but that type was not returned as a job dependency. To ensure correct behavior of other systems, " +
