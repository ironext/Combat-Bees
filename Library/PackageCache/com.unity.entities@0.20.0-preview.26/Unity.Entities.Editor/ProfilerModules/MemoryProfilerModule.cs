...
namespace Unity.Entities.Editor
{
    partial class MemoryProfilerModule : MemoryProfilerModuleBase
    {
        ...
        const int k_ProfilerWindowMinWidth = 720;
        const int k_RightPaneMinWidth = 250;
        ...
        public override VisualElement CreateView(Rect area)
        {
            ...
            var splitter = m_Window.Q<TwoPaneSplitView>("splitter");
            splitter.fixedPaneInitialDimension = math.max(area.width - k_RightPaneMinWidth, k_ProfilerWindowMinWidth - k_RightPaneMinWidth);
            CreateViewLeftPane(splitter.Q("left-pane"));
            CreateViewRightPane(splitter.Q("right-pane"));
        ...
        void CreateViewRightPane(VisualElement root)
        {
            ...
            m_RightPane.SetVisibility(false);
        ...
        public override void SelectedFrameIndexChanged(long index)
        {
            ...
            m_ArchetypesDataSource = GetFrames(index).SelectMany(GetTreeViewData).ToArray();
            m_SearchElement.Search();
        }
        ...
        public override void Update()
        {
            ...
            m_TreeView.rootItems = rootItem.hasChildren ? new[] { rootItem } : Array.Empty<MemoryProfilerTreeViewItem>();
        ...
        void SetInspectorValue(MemoryProfilerTreeViewItem item)
        {
            if (item != null && item.data.ComponentTypes != null)
            {
                ...
                m_RightPane.SetVisibility(true);
            }
            else
            {
                m_RightPane.SetVisibility(false);
