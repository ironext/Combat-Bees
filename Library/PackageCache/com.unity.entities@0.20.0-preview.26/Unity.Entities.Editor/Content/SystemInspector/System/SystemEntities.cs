...
namespace Unity.Entities.Editor
{
    class SystemEntities
    {
        ...
        public SystemEntities(World world, SystemProxy system)
        {
            m_World = world;
            m_System = system;
        ...
        public unsafe List<QueryWithEntitiesViewData> EntitiesFromQueries
        {
            get
            {
                if (!m_World.IsCreated || !m_System.Valid)
                {
                    m_EntitiesFromQueries.Clear();
                    return m_EntitiesFromQueries;
                }

                var ptr = m_System.StatePointerForQueryResults;
                if (ptr == null)
                {
                    m_EntitiesFromQueries.Clear();
                    return m_EntitiesFromQueries;
                ...
                {
                    m_EntitiesFromQueries.Add(new QueryWithEntitiesViewData(m_World, m_LastEntityQueries[i], m_System.NicifiedDisplayName, i + 1));
                }
