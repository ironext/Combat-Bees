...
namespace Unity.Entities.Editor
{
    public class StartLiveLinkWindow : EditorWindow
    {
        ...
        internal class StartLiveLinkView
        {
            ...
            static readonly VisualElementTemplate s_WindowTemplate = PackageResources.LoadTemplate("LiveLink/live-link-window");
            static readonly VisualElementTemplate s_WindowItemTemplate = PackageResources.LoadTemplate("LiveLink/live-link-window-item");
            static readonly List<StartMode> s_StartModes = new List<StartMode> { StartMode.RunLatestBuild, StartMode.BuildAndRun, StartMode.Build };
            ...
            public void Initialize(VisualElement rootVisualElement)
            {
                ...
                m_StartModeDropdown = new PopupField<StartMode>(s_StartModes, StartMode.RunLatestBuild, FormatStartMode, FormatStartMode) { style = { flexGrow = 1 } };
            ...
            public void SetEnableBuildingState(bool isBuilding)
            {
                ...
                m_ActionButtons.SetVisibility(!isBuilding);
                m_BuildMessage.SetVisibility(isBuilding);
            }
            ...
            void OnConfigurationListViewSelectionChange(IEnumerable<object> obj)
            {
                ...
                m_StartModeDropdown.index = m_SelectedConfiguration != null ? s_StartModes.IndexOf(m_SelectedConfiguration.SelectedStartMode) : 0;
            ...
            {
                var e = s_WindowItemTemplate.Clone();
            ...
            void BindBuildConfigurationItem(VisualElement visualElement, int index)
            {
                ...
                errorIcon.SetVisibility(!canBuild);
                errorIcon.tooltip = canBuild ? string.Empty : $"This Build Configuration as an error preventing it from being built: {Environment.NewLine}{StripAnyHtmlTag(canBuild.Message)}";
            ...
            public bool IsActionAllowed(StartMode mode, out string reason)
            {
                ...
                switch (mode)
                {
                    case StartMode.RunLatestBuild:
                    {
                        ...
                        {
                            reason = $"Impossible to run latest build: {canRun.Message}";
                            return false;
                        }
                        return true;
                    }
                    ...
                    case StartMode.Build:
                    {
                        ...
                        if (canBuild)
                            return true;
                        reason = $"Impossible to build this configuration: {canBuild.Message}";
                        return false;
                    }
                    default:
                        throw new ArgumentOutOfRangeException(nameof(mode), mode, null);
