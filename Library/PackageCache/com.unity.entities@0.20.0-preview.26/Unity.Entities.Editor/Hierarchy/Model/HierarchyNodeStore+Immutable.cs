...
namespace Unity.Entities.Editor
{
    ...
    partial struct HierarchyNodeStore
    {
        ...
        public struct ExportImmutableEnumerator : IEnumerator
        {
            ...
            public ExportImmutableEnumerator(HierarchyNodeStore hierarchy, EntityManager entityManager, ExportImmutableState state, Immutable write, Immutable read, int batchSize)
            {
            ...
            public bool MoveNext()
            {
                ...
                switch (m_Step)
                {
                    case Step.HierarchyNodes:
                    {
                        ...
                        {
                            EntityCapacity = m_EntityManager.EntityCapacity,
        ...
        public void ExportImmutable(EntityManager entityManager, Immutable dstBuffer)
        {
            using var srcBuffer = new Immutable(Allocator.TempJob);
            using var state = new ExportImmutableState(Allocator.TempJob);
            
            var enumerator = CreateBuildImmutableEnumerator(entityManager, state, dstBuffer, srcBuffer, 0);
            while (enumerator.MoveNext())
            {
            }
        }

        public void ExportImmutable(EntityManager entityManager, Immutable dstBuffer, Immutable srcBuffer)
        {
            using var state = new ExportImmutableState(Allocator.TempJob);
        ...
        /// </summary>
        /// <param name="entityManager">The entity manager backing the data.</param>
        ...
        public ExportImmutableEnumerator CreateBuildImmutableEnumerator(EntityManager entityManager, ExportImmutableState state, Immutable dstBuffer, Immutable srcBuffer, int batchSize)
        {
            return new ExportImmutableEnumerator(this, entityManager, state, dstBuffer, srcBuffer, batchSize);
        }
