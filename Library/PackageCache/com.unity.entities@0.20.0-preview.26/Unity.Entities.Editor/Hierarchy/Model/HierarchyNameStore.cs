    ...
    unsafe class HierarchyNameStore : IDisposable
    {
        ...
        public void IntegrateEntityChanges(HierarchyEntityChanges changes)
        {
            for (var i = 0; i < changes.RemovedSceneReferencesEntities.Length; i++)
                RemoveName(HierarchyNodeHandle.FromSubScene(changes.RemovedSceneReferencesEntities[i]));

            for (var i = 0; i < changes.AddedSceneReferencesEntities.Length; i++)
                SetName(HierarchyNodeHandle.FromSubScene(changes.AddedSceneReferencesEntities[i]), GetSubSceneName(changes.AddedSceneReferencesEntities[i]));
        }
