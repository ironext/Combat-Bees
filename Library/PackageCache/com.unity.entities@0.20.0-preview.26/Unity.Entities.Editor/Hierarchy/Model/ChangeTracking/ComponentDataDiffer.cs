...
namespace Unity.Entities.Editor
{
    class ComponentDataDiffer : IDisposable
    {
        ...
        public unsafe ComponentChanges GatherComponentChangesAsync(EntityQuery query, Allocator allocator, out JobHandle jobHandle)
        {
            using (k_GatherComponentChangesAsync.Auto())
            {
                ...
                var concatResultJobHandle = new ConcatResultJob
                {
                    ComponentSize = m_ComponentSize,
                    Allocator = allocator,
                    GatheredChanges = m_GatheredChanges.GetUnsafeList(),
        ...
        unsafe struct GatherComponentChangesJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                if (ShadowChunksBySequenceNumber.TryGetValue(chunk->SequenceNumber, out var shadow))
                {
                    ...
                    {
                        changesForChunk->AddedComponentEntities = new UnsafeList<Entity>(0, Allocator.TempJob);
                        changesForChunk->AddedComponentDataBuffer = new UnsafeList<byte>(0, Allocator.TempJob);
                        changesForChunk->RemovedComponentEntities = new UnsafeList<Entity>(0, Allocator.TempJob);
                        changesForChunk->RemovedComponentDataBuffer = new UnsafeList<byte>(0, Allocator.TempJob);
                    }
            ...
            static void OnNewComponent(ChangesCollector* changesForChunk, Entity entity, byte* currentComponentData, int componentSize)
            {
                ...
                changesForChunk->AddedComponentDataBuffer.AddRange(currentComponentData, componentSize);
            }
            ...
            static void OnRemovedComponent(ChangesCollector* changesForChunk, Entity entity, byte* previousComponentData, int componentSize)
            {
                ...
                changesForChunk->RemovedComponentDataBuffer.AddRange(previousComponentData, componentSize);
            }
        }
        ...
        unsafe struct ConcatResultJob : IJob
        {
            ...
            [NativeDisableUnsafePtrRestriction] public UnsafeList<ChangesCollector>* GatheredChanges;
            ...
            public void Execute()
            {
                ...
                {
                    ref var changesForChunk = ref GatheredChanges->ElementAt(i);
                ...
                for (var i = 0; i < GatheredChanges->Length; i++)
                {
                    ...
                    {
                        buffer.AddRangeNoResize(changesForChunk.AddedComponentDataBuffer.Ptr, changesForChunk.AddedComponentDataBuffer.Length);
                ...
                {
                    ref var changesForChunk = ref GatheredChanges->ElementAt(chunksWithRemovedData[i]);
                    buffer.AddRangeNoResize(changesForChunk.RemovedComponentDataBuffer.Ptr, changesForChunk.RemovedComponentDataBuffer.Length);
                ...
                buffer.AddRangeNoResize(RemovedChunkComponentDataBuffer.GetUnsafeReadOnlyPtr(), RemovedChunkComponentDataBuffer.Length);
        ...
        {
            public UnsafeList<byte> AddedComponentDataBuffer;
            public UnsafeList<byte> RemovedComponentDataBuffer;
        ...
        internal readonly unsafe struct ComponentChanges : IDisposable
        {
            ...
            {
                if (!m_Result.Value.Buffer.IsCreated) return;
