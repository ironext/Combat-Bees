...
namespace Unity.Entities.Editor
{
    class SystemTreeView : VisualElement, IDisposable
    {
        ...
        void RecreateTreeViewRootItems()
        {
            ...
            if (World.All.Count > 0 && string.IsNullOrEmpty(SearchFilter.ErrorComponentType))
            {
                ...
                foreach (var node in graph.Roots)
                {
                    ...
                    var item = SystemTreeViewItem.Acquire((PlayerLoopSystemGraph)graph, node, null, m_World);
        ...
        void PopulateListViewWithSearchResults()
        {
            ...
            {
                var listViewItems = SystemTreeViewItem.Acquire(LocalSystemGraph, system.Node, null, m_World);
        ...
        {
            if (SelectedSystem.Valid && SelectedSystem.World == m_World && m_AllSystemsForSearch.Count > 0)
            {
