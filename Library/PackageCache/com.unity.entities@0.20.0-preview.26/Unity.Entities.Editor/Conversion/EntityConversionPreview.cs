...
namespace Unity.Entities.Editor
{
    ...
    class EntityConversionPreview : ObjectPreview
    {
        ...
        static readonly GUIContent k_PreviewTitle = EditorGUIUtility.TrTextContent("Entity Conversion");
        ...
        static class Strings
        {
            ...
            public static readonly string LiveConversionDisabled = L10n.Tr("Live Conversion is disabled. Enable it in the DOTS file menu to see conversion preview.");
            public static readonly string PreviewForUnconvertedGameObjects = L10n.Tr("Entity conversion can only be previewed for GameObjects converted by a SubScene.");
            public static readonly string MultiSelectionWarning = L10n.Tr("Components that are only on some of the converted entities are not shown.");
            public static readonly string WillBeConvertedAtRuntime = L10n.Tr("This game object will be converted at runtime.");
            public static readonly string PrimaryEntityDestroyed = L10n.Tr("The primary entity has been destroyed during conversion.");
            public static readonly string OneOrMorePrimaryEntitiesDestroyed = L10n.Tr("One or more primary entities have been destroyed during conversion.");
            public static readonly string TooManyAdditionalEntities = L10n.Tr("Too many additional entities to display.");
            public static readonly string TooManyGameObjectsToPreview = L10n.Tr($"For performance reasons, conversion preview is disabled when inspecting more than {k_MaxPreviewableGameObjectsCount} GameObjects.");
        }
        ...
        readonly List<EntityConversionData> m_CachedConversionData = new List<EntityConversionData>();
        ...
        public override void OnPreviewGUI(Rect r, GUIStyle background)
        {
            ...
            using (new EditorGUILayout.VerticalScope())
            {
                ...
                if (!LiveLinkConfigHelper.LiveLinkEnabledInEditMode)
                {
                ...
                if (firstLayout)
                    EntityConversionUtility.GetConversionData(m_GameObjectTargets, GetCurrentlySelectedWorld(), m_CachedConversionData);

                if (m_GameObjectTargets.Count != m_CachedConversionData.Count)
                {

                    GUILayout.Label(EditorGUIUtilityBridge.TempContent(Strings.PreviewForUnconvertedGameObjects), Styles.EntityConversionWarningMessage);
                    return;
                }

                if (!ValidateGameObjectConversionData(m_CachedConversionData, out var errorMessage))
                {
                    GUILayout.Label(EditorGUIUtilityBridge.TempContent(errorMessage), Styles.EntityConversionWarningMessage);
                    return;
                }

                var primary = m_CachedConversionData[0];
                var isSingleSelection = m_CachedConversionData.Count == 1;
                ...
                var entityName = isSingleSelection
                    ...
                    : $"{EditorGUIBridge.mixedValueContent.text}({m_CachedConversionData.Count} entities)";
                ...
                if (firstLayout)
                {
                    GetInspectorTargets(m_CachedConversionData, m_State, m_InspectorTargets);
        ...
        static bool ValidateGameObjectConversionData(IReadOnlyList<EntityConversionData> conversionData, out string message)
        {
            ...
            {
                message = Strings.WillBeConvertedAtRuntime;
                return false;
            }
        ...
        static void GetInspectorTargets(IReadOnlyList<EntityConversionData> conversionData, State state, List<EntityContainer> result)
        {
            result.Clear();
        ...
        class GameObjectConversionChangeTracker
        {
            ...
            unsafe bool DidChangeInternal(World world, IEnumerable<GameObject> targets, IEnumerable<int> selectedComponentTypes)
            {
                ...
                if (m_LastConversionData == EntityConversionData.Null)
                {
                    ...
                    m_LastConversionData = EntityConversionUtility.GetConversionData(targets.FirstOrDefault(), world);
