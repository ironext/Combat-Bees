...
namespace Unity.Scenes.Editor.Tests
{
    ...
    {
        [SerializeField] TestWithEditorLiveLink m_EditorLiveLink;
        [SerializeField] bool m_debugLoggingWasEnabled;

        [OneTimeSetUp]
        public void OneTimeSetUp()
        {
            if (!m_EditorLiveLink.OneTimeSetUp())
                return;
        ...
        {
            m_EditorLiveLink.OneTimeTearDown();
            EditorSceneManager.NewScene(NewSceneSetup.EmptyScene);
            LiveConversionSettings.IsDebugLoggingEnabled = m_debugLoggingWasEnabled;
        }

        [SetUp]
        public void SetUp() => m_EditorLiveLink.SetUp();

        [UnityTest]
        public IEnumerator IncrementalConversion_WithSingleObject_LogsAreCorrect()
        {
            var subScene = m_EditorLiveLink.CreateEmptySubScene("TestSubScene", true);
            SceneManager.SetActiveScene(subScene.EditingScene);

            var w = m_EditorLiveLink.GetLiveLinkWorldForEditMode();
            yield return m_EditorLiveLink.UpdateEditorAndWorld(w);
            {
