...
namespace Unity.Scenes
{
    internal class EntityScenesPaths
    {
        ...
        internal static string GetLiveLinkCachePath(UnityEngine.Hash128 targetHash, PathType type, int sectionIndex)
        {
#if UNITY_DOTSRUNTIME
            var loadDir = "Data";
#else
            var loadDir = Application.persistentDataPath;
#endif
            var extension = GetExtension(type);
            switch (type)
            {
                case PathType.EntitiesHeader:
                    return $"{loadDir}/{k_LiveLinkCacheDir}/{targetHash}.{extension}";
                case PathType.EntitiesBinary:
                case PathType.EntitiesUnityObjectReferences:
                case PathType.EntitiesUnityObjectReferencesBundle:
                    return $"{loadDir}/{k_LiveLinkCacheDir}/{targetHash}.{sectionIndex}.{extension}";
                default:
                    return "";
            }
        }
        ...
        internal static string GetCachePath(Hash128 targetHash)
        {
#if UNITY_DOTSRUNTIME
            var loadDir = "Data";
#else
            var loadDir = Application.persistentDataPath;
#endif
            return $"{loadDir}/{k_LiveLinkCacheDir}/{targetHash}";
        }

        internal static string GetTempCachePath()
        {
#if UNITY_DOTSRUNTIME
            var loadDir = "Data";
#else
            var loadDir = Application.persistentDataPath;
#endif
            return $"{loadDir}/{k_LiveLinkCacheDir}/{MakeRandomFileName()}";
        }
        ...
        internal static string ComposeLiveLinkCachePath(string fileName)
        {
#if UNITY_DOTSRUNTIME
            var loadDir = "Data";
#else
            var loadDir = Application.persistentDataPath;
#endif
            return $"{loadDir}/{k_LiveLinkCacheDir}/{fileName}";
        }

        internal static string GetLiveLinkCacheDirPath()
        {
#if UNITY_DOTSRUNTIME
            var loadDir = "Data";
#else
            var loadDir = Application.persistentDataPath;
#endif
            return $"{loadDir}/{k_LiveLinkCacheDir}";
        }
