    ...
    class LiveLinkPlayerAssetRefreshSystem : ComponentSystem
    {
        ...
        static unsafe void ReceiveBuildArtifact(MessageEventArgs args)
        {
            fixed(byte* ptr = args.data)
            {
                ...
                reader.ReadNextNBC(out string artifactFileName);
        ...
        unsafe bool IsSubSceneAvailable(in ResolvedSubSceneID subSceneId)
        {
            ...
            if (!ResolveSceneSectionUtility.ReadHeader(headerPath, out var sceneMetaDataRef, subSceneId.SubSceneGUID.Guid))
            {
                return false;
            }
        ...
        void AddWaitForSubScene(in ResolvedSubSceneID subSceneId)
        {
        ...
        protected override void OnUpdate()
        {
            ...
            // We only process and load Entity Scenes if we are waiting on no assets
            if (m_WaitingForSubScenes.Count != 0 && m_WaitingForAssets.Count == 0)
            {
                ...
                {
                    if (!subScene.Value.TargetHash.IsValid)
                    {
