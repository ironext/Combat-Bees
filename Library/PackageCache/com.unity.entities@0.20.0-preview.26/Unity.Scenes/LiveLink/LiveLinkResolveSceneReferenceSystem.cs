...
namespace Unity.Scenes
{
    ...
    class LiveLinkResolveSceneReferenceSystem : ComponentSystem
    {
        ...
        void ResolveScene(Entity sceneEntity, ref SceneReference scene, RequestSceneLoaded requestSceneLoaded, Hash128 artifactHash)
        {
            ...
            if (!ResolveSceneSectionUtility.ReadHeader(sceneHeaderPath, out var sceneMetaDataRef, scene.SceneGUID, out var headerBlobOwner))
            {
                return;
            }
            ...
            for (int i = 0; i != sceneMetaData.Sections.Length; i++)
            {
                ...
                var sectionData = sceneMetaData.Sections[i];

                EntityManager.AddComponentData(sectionEntity, sectionData);
