...
#pragma warning restore 649
        ...
        [Test]
        public void EntityName_GetName_FixedString_Truncate()
        {
            //134 characters
            var reallyLongStringName = "A string that is longer than 61 characters, such that it cannot" +
                                      " fit into a FixedString64 and a truncated return can be properly tested";

            var truncatedStringResult = reallyLongStringName.Substring(0, EntityComponentStore.k_MaxDebugNameLength);

            var archetype = m_Manager.CreateArchetype();
            var entity = m_Manager.CreateEntity(archetype);

            m_Manager.SetName(entity,reallyLongStringName);

            // Retrieving a managed string should give you the full untruncated name
            var fullName = m_Manager.GetName(entity);
            Assert.AreEqual(reallyLongStringName,fullName);

            // Retrieving a FixedString64 should give you a truncated name
            m_Manager.GetName(entity, out var fixedName);
            Assert.AreEqual(truncatedStringResult,fixedName);
        }
